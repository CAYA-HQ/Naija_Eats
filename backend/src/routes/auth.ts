...
router.post("/register", async (req: Request, res: Response) => {
  try {
    ...
    if (phoneError && !phoneError.message.includes("SMS provider")) {
      return _res.error(400, res, phoneError.message);
    }
...
router.post("/login", async (req: Request, res: Response) => {
  try {
    ...
    if (!email || !password)
      return _res.error(400, res, "Email and password are required");
    ...
    return _res.success(201, res, "User logged in successfully", {
